...
const { wsAuthenticate } = require('../../util/util');
...
const start = (io) => {
  io.of(/^\/[0-9]+$/)
    ...
    .on('connection', async (socket) => {
      ...
      const { firstChild, files, revisionID } = await getFileSystem(
        vaultID,
        socket.userID
      );
      ...
      socket
        ...
        .on('operation', async (clientRevisionID, operation) => {
          ...
          socket
            .to(socket.fileID)
            .emit('syncOp', revisionID, operation, socket.id);
          LogOp[fileID][revisionID] = operation;
          if (
            onlines[fileID].isSaved &&
            onlines[fileID].revisionID != revisionID
          ) {
            fileArr[fileID].recordID = await createFileVersion(
              fileID,
              revisionID,
              doc
            );
