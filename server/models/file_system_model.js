...
const getFileSystem = async (vaultID, userID) => {
  ...
  try {
    const [vault] = await conn.query(
      'SELECT first_child_id, revision_id from vaults where id = ?',
      [vaultID]
    );
    const [files] = await conn.query(
      'SELECT id, name, type, first_child_id as firstChild, next_id as next from folder_file where vault_id = ?',
      [vaultID]
    );
    await conn.query(
...
const getFile = async (fileID) => {
  try {
    const [file] = await pool.query(
      'SELECT id, text, revision_id, name FROM files WHERE file_id = ? ORDER BY revision_id DESC LIMIT 1',
      [fileID]
    );
    if (file.length < 1) {
...
const createFile = async (
  ...
  try {
    ...
    const [vaults] = await conn.query(
      `SELECT revision_id FROM vaults WHERE id = ? FOR UPDATE`,
      [vaultID]
    );
    ...
    if (type == DATA_TYPE.FILE) {
      await conn.query(
        'INSERT INTO files (file_id, revision_id, text, updated_at) VALUES (?, ?, ?, ?)',
        [result.insertId, 0, '', newFile['created_at']]
      );
    }
    if (prevID) {
      await conn.query('UPDATE folder_file SET next_id = ? WHERE id = ?', [
        result.insertId,
        prevID
      ]);
    } else if (parentID) {
      await conn.query(
        'UPDATE folder_file SET first_child_id = ? WHERE id = ?',
        [result.insertId, parentID]
      );
    } else {
      await conn.query('UPDATE vaults SET first_child_id = ? WHERE id = ?', [
        result.insertId,
        vaultID
      ]);
    }
...
const changeFileName = async (fileID, name) => {
  try {
    await pool.query('UPDATE folder_file SET name = ? WHERE id = ?', [
      name,
      fileID
    ]);
  } catch (e) {
...
const searchFiles = async (userID, vaultID, keyword) => {
  ...
  try {
    ...
    const [textResult] = await conn.query(
      ...
      SELECT id
      FROM t
      WHERE rank_no = 1 AND lower(text) LIKE ?`,
      [vaultID, `%${keyword}%`]
    );
    const [nameResult] = await conn.query(
      `SELECT id FROM folder_file WHERE vault_id = ? and lower(name) LIKE ? and type = ?`,
      [vaultID, `%${keyword}%`, DATA_TYPE.FILE]
    );
...
const getFileVersionHistory = async (fileID) => {
  try {
    const [files] = await pool.query(
      'SELECT revision_id, updated_at, name FROM files WHERE file_id = ? ORDER BY revision_id DESC',
      [fileID]
    );
    return { files };
  } catch (error) {
...
const getFileVersion = async (fileID, revisionID) => {
  try {
    const [file] = await pool.query(
      'SELECT text FROM files WHERE file_id = ? and revision_id = ?',
      [fileID, revisionID]
    );
    if (file.length == 0) {
      return { error: 'Database query error' };
...
const changeFileVersionName = async (fileID, revisionID, text, name) => {
  try {
    ...
    await pool.query(
      'INSERT INTO files (file_id, revision_id, text, name, updated_at) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = ?',
      [fileID, revisionID, text, name, updatedAt, name]
    );
    return {};
  } catch (error) {
    console.log(error);
...
const restoreFileVersion = async (fileID, revisionID) => {
  ...
  try {
    const [file] = await conn.query(
      'SELECT id, text FROM files WHERE file_id = ?  and revision_id = ?',
      [fileID, revisionID]
    );
    await conn.query(
      'DELETE FROM files WHERE file_id = ? and revision_id > ?',
      [fileID, revisionID]
    );
    if (file.length == 0) {
      return { error: 'Database query error' };
...
const createFileVersion = async (fileID, revisionID, doc, updatedAt) => {
  try {
    const [result] = await pool.query(
      'INSERT INTO files ( file_id, revision_id, text, updated_at) VALUES (?, ?, ?, ?)',
      [fileID, revisionID, doc, updatedAt]
    );
...
const updateFileVersion = async (
  ...
  try {
    await pool.query(
      'UPDATE files SET revision_id = ?, text = ?, updated_at = ? WHERE id = ?',
      [revisionID, doc, updatedAt, recordID]
    );
  } catch (e) {
    console.log(e);
  }
