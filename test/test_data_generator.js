...
async function _getFakeVault(conn) {
  const [vaults] = await conn.query(
    'SELECT vault_id FROM vault_user WHERE user_id = ?',
    [users[0].id]
  );
...
async function _createFakeFile(conn) {
  ...
  file.id = fileId;
  await conn.query(
    'INSERT INTO files (file_id, revision_id, text) VALUES (?, 0, "")',
    [fileId]
  );
  return await conn.query('UPDATE vaults SET first_child_id = ? WHERE id = ?', [
    vault.id,
    fileId
  ]);
}
